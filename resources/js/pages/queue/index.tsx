...
export default function QueuePage({ context, queues, filters }: QueuePageProps) {
    ...
    if (isAdminWorkspace) {
        return <AdminQueuePage context={context} queues={queues} filters={filters} page={page} />;
    }

    return <ReadOnlyQueuePage context={context} queues={queues} filters={filters} page={page} />;
}

function AdminQueuePage({ context, queues, filters, page }: { context: WorkspaceContext; queues: QueueEntry[]; filters: QueuePageProps['filters']; page: ReturnType<typeof usePage<SharedData & { errors?: ValidationErrors }>> }) {
...
function ActionButton({ children, disabled, onClick, variant = 'secondary', danger = false, className = '' }: { children: string; disabled: boolean; onClick: () => void; variant?: 'primary' | 'secondary'; danger?: boolean; className?: string }) {
