...
export default function DoctorListPage({ doctors, filters }: DoctorListProps) {
    ...
    const filteredDoctors = useMemo(
        () =>
            doctors.filter((doctor) => {
                const clinicNames = doctor.clinics.map((clinic) => clinic.name);
                const clinicCities = doctor.clinics.map((clinic) => clinic.city_name);
                const cityMatches =
                    selectedCities.length === 0 ||
                    clinicCities.some((city) => selectedCities.some((selected) => selected.toLowerCase() === (city ?? '').toLowerCase()));
                const specialityMatches =
                    selectedSpecialities.length === 0 ||
                    doctor.specialities.some((speciality) => selectedSpecialities.some((selected) => selected.toLowerCase() === speciality.toLowerCase()));

                return (
                    matchesText(search, [doctor.name, doctor.email, doctor.phone_number, ...doctor.specialities, ...clinicNames, ...clinicCities]) &&
                    cityMatches &&
                    specialityMatches &&
                    matchesPublicTimeFilter(time, doctor.time_ranges)
                );
            }),
        [doctors, search, selectedCities, selectedSpecialities, time],
    );
        ...
        <>
            ...
            <div className="min-h-screen bg-[#DED0B6] text-[#40311D]">
                ...
                <main className="mx-auto w-full max-w-[1400px] px-5 pb-24 pt-12 md:px-8">
                    <PageHeader
                        title="Cari Dokter"
                        placeholder="Cari nama dokter, spesialisasi, atau tempat klinik"
                        search={search}
                    ...
                    <div className="grid items-start gap-8 lg:grid-cols-[200px_minmax(0,1fr)]">
                        ...
                        {filteredDoctors.length === 0 ? (
                            <EmptyDirectory>Tidak ada dokter yang sesuai dengan filter saat ini.</EmptyDirectory>
                        ) : (
                ...
                <FilterModal open={filtersOpen} resultCount={filteredDoctors.length} onClose={() => setFiltersOpen(false)} onReset={resetFilters}>
                    {filterPanel}
                </FilterModal>
            </div>
        </>
...
function DoctorDirectoryCard({ doctor }: { doctor: PublicDoctor }) {
    ...
    const tagItems = [doctor.primary_speciality, cityName].filter(Boolean) as string[];
        ...
        >
            ...
            <div className="min-w-0">
                <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                        <h2 className="line-clamp-2 text-[15px] font-bold leading-tight tracking-[-0.03em] group-hover:text-[#00917B]">{doctor.name}</h2>
                        <p className="mt-0.5 line-clamp-1 text-xs font-extrabold text-[#40311D]/65">{primaryClinic?.name ?? 'Klinik belum tersedia'}</p>
                    </div>
                    <SmallStatusBadge active={doctor.is_available_today}>{doctor.is_available_today ? 'Tersedia' : 'Tidak Tersedia'}</SmallStatusBadge>
                </div>
                <div className="mt-1 text-xs font-medium leading-relaxed text-[#40311D]/55">
                    <p>{doctor.phone_number ?? '-'}</p>
                    <p className="line-clamp-1">{doctor.email ?? '-'}</p>
                </div>

                <p className="mt-4 text-xs font-medium text-[#40311D]/50">
                    Jam Operasional : <span className="font-bold text-[#40311D]/65">{doctor.operational_label}, {doctor.hours_label}</span>
                </p>

                <TagList items={tagItems} fallback="Dokter klinik" />
            </div>
        </Link>
    );
}
