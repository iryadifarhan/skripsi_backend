...
export default function DoctorDetail({ doctor }: DoctorDetailProps) {
    ...
    const reservationTarget = selectedClinic
        ? patientHref(`/reservasi?booking_clinic_id=${selectedClinic.id}&booking_doctor_id=${doctor.id}`)
        : patientHref(`/reservasi?booking_doctor_id=${doctor.id}`);
    const activeStep = selectedClinic ? (selectedDay ? (selectedSlot ? (complaint.trim() ? 4 : 3) : 2) : 1) : 0;
    ...
    const canBook = Boolean(selectedClinic && selectedDay && selectedSlot && complaint.trim());
