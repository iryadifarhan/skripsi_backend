                                    ...
                                    <Panel title="Ringkasan Dokter" subtitle="Detail dokter terpilih">
                                        {selectedDoctor === null ? (
                                            ...
                                            <div className="space-y-3 text-[12px] text-gray-600">
                                                <div className="flex items-center gap-3">
                                                    <DoctorAvatar doctor={selectedDoctor} large />
                                                    <div>
...
function DoctorAvatar({ doctor, large = false }: { doctor: DoctorEntry; large?: boolean }) {
    const size = large ? 'h-14 w-14' : 'h-10 w-10';

    if (doctor.image_url) {
        return <img src={doctor.image_url} alt={doctor.name} className={`${size} rounded-lg object-cover`} />;
    }

    return (
        <div className={`flex ${size} items-center justify-center rounded-lg bg-[#40311D] text-[11px] font-medium text-white`}>
