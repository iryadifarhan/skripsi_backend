...
export default function DoctorEditPage({ context, clinicId, clinic, doctor: initialDoctor, isClinicDoctor, schedules, medicalRecords, canViewMedicalRecords }: DoctorEditPageProps) {
                            ...
                            <Panel title="Profil Dokter" subtitle="Foto identitas">
                                <div className="flex min-h-[420px] flex-col gap-6">
                                    <div className="flex items-center gap-4">
                                        {doctor.image_url ? (
                                            <img src={doctor.image_url} alt={doctor.name} className="h-24 w-24 rounded-xl object-cover" />
                                        ) : (
                                            <div className="flex h-24 w-24 items-center justify-center rounded-xl bg-[#40311D] text-2xl font-medium text-white">
                                                {doctor.name.slice(0, 2).toUpperCase()}
                                            </div>
                                        )}
                                        <div className="min-w-0">
                                            <p className="truncate text-[15px] font-medium text-[#2c2115]">{doctor.name}</p>
                                            <p className="truncate text-[12px] text-gray-500">{doctor.email}</p>
                                            <span className={`mt-2 inline-block rounded-full px-2 py-0.5 text-[10px] font-medium ${isClinicDoctor ? 'bg-teal-50 text-teal-700' : 'bg-amber-50 text-amber-700'}`}>
                                                {isClinicDoctor ? 'Terdaftar di klinik' : 'Belum terdaftar di klinik'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    {localImagePreviewUrl ? (
                                        <div className="relative">
                                            ...
                                            <img
                                                ...
                                                alt={`Preview foto baru ${doctor.name}`}
                                                className="h-52 w-full rounded-xl border border-[#e4ddd4] bg-[#faf9f7] object-contain p-2"
                                            />
                                        </div>
                                    ) : null}
                                    ...
                                    <div className="space-y-3">
                                        <input
                                            ...
                                            className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-[12px]"
                                        />
                                        <button
                                            type="button"
                                            onClick={uploadImage}
