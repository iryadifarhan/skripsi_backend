        ...
        <>
            <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                    <PatientAvatar name={row.name} imageUrl={row.image_url} />
                    <div>
...
function PatientSummaryPanel({ entry, selectedClinicId }: { entry: PatientRow; selectedClinicId: number | null }) {
    ...
    return (
        <div className="grid gap-4">
            <div className="flex items-center gap-3">
                {isRegistered ? <PatientAvatar name={entry.name} imageUrl={(entry as PatientSummaryEntry).image_url} /> : <PatientAvatar name={entry.name} />}
                <div>
...
function PatientAvatar({ name, imageUrl }: { name: string; imageUrl?: string | null }) {
    if (imageUrl) {
        return <img src={imageUrl} alt={name} className="h-12 w-12 rounded-lg object-cover" />;
    }

    return (
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-[#40311D] text-[11px] font-bold uppercase text-white">
            {initials(name)}
        </div>
    );
}
