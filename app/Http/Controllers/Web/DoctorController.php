...
class DoctorController extends Controller
{
    ...
    public function edit(Request $request, int $doctor): Response
    {
        ...
        return Inertia::render('doctors/edit', [
            ...
            'profilePictureOptions' => User::profilePicturesForRole(User::ROLE_DOCTOR),
        ]);
    }
    ...
    public function deleteImage(Request $request, int $doctor): RedirectResponse
    {
        $this->authorizeAdminWorkspace($request);
        ...
        $doctorModel = $this->findEditableDoctor($request, $clinic, $doctor);
        $this->deleteUploadedImage($doctorModel);
        $doctorModel->update([
            'image_path' => null,
        ]);
        ...
        return to_route('doctors.edit', [
            'doctor' => $doctorModel->id,
            'clinic_id' => $clinic->id,
        ])->with('status', 'Foto upload dokter berhasil dihapus.');
    }

    public function updateProfilePicture(Request $request, int $doctor): RedirectResponse
    {
        $this->authorizeAdminWorkspace($request);
        ...
        $payload = $request->validate([
            ...
            'profile_picture' => [
                ...
                'nullable',
                'string',
                Rule::in(User::profilePicturesForRole(User::ROLE_DOCTOR)),
            ],
        ]);
        ...
        if ($profilePicture === null && !filled($doctorModel->image_path)) {
        ...
        return to_route('doctors.edit', [
            'doctor' => $doctorModel->id,
            'clinic_id' => $clinic->id,
        ])->with('status', 'Avatar dokter berhasil diperbarui.');
    }
    ...
    private function findEditableDoctor(Request $request, Clinic $clinic, int $doctor): User
    {
        if ($request->user()->role === User::ROLE_SUPERADMIN) {
            $doctorModel = User::query()
                ->whereKey($doctor)
                ->where('role', User::ROLE_DOCTOR)
                ->first(['id', 'name', 'username', 'email', 'phone_number', 'date_of_birth', 'gender', 'profile_picture', 'image_path', 'role']);
    ...
    private function findClinicDoctor(Clinic $clinic, int $doctor): User
    {
        $doctorModel = $clinic->doctors()
            ...
            ->first(['users.id', 'users.name', 'users.username', 'users.email', 'users.phone_number', 'users.date_of_birth', 'users.gender', 'users.profile_picture', 'users.image_path', 'users.role']);
    ...
    private function unassignedDoctors(Clinic $clinic): array
    {
        return User::query()
            ...
            ->orderBy('name')
            ->get(['id', 'name', 'username', 'email', 'phone_number', 'date_of_birth', 'gender', 'profile_picture', 'image_path', 'role'])
