...
class PatientController extends Controller
{
    ...
    public function edit(Request $request, int $patient): Response
    {
        ...
        return Inertia::render('patients/edit', [
            ...
            'profilePictureOptions' => User::profilePicturesForRole(User::ROLE_PATIENT),
        ]);
    }
    ...
    public function updateProfilePicture(Request $request, int $patient): RedirectResponse
    {
        $this->authorizeSuperadmin($request);

        $patientModel = $this->findPatientForMutation($patient);
        $payload = $request->validate([
            'profile_picture' => [
                'present',
                'nullable',
                'string',
                Rule::in(User::profilePicturesForRole(User::ROLE_PATIENT)),
            ],
        ]);

        $profilePicture = $payload['profile_picture'] ?? null;

        if ($profilePicture === null && !filled($patientModel->image_path)) {
        ...
        return to_route('patients.edit', $patientModel)->with('status', 'Avatar pasien berhasil diperbarui.');
    }

    public function uploadImage(Request $request, int $patient): RedirectResponse
    {
        $this->authorizeSuperadmin($request);
    ...
    public function deleteImage(Request $request, int $patient): RedirectResponse
    {
        $this->authorizeSuperadmin($request);

        $patientModel = $this->findPatientForMutation($patient);
        $this->deleteUploadedImage($patientModel);
        $patientModel->update([
            'image_path' => null,
        ]);

        return to_route('patients.edit', $patientModel)->with('status', 'Foto upload pasien berhasil dihapus.');
    }
    ...
    private function findPatientForMutation(int $patient): User
    {
        return User::query()
            ->whereKey($patient)
            ->where('role', User::ROLE_PATIENT)
    ...
    private function registeredPatients(Request $request, ?int $clinicId): array
    {
        ...
        return $query->get(['id', 'name', 'username', 'email', 'phone_number', 'date_of_birth', 'gender', 'profile_picture', 'image_path', 'role'])
    ...
    private function serializePatientSummary(User $patient, string $type): array
    {
        return [
            ...
            'display_avatar_url' => $patient->display_avatar_url,
